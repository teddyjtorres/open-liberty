...
public class LibertyMirrorSubstitutorTest {
    ...
    public void testWithArtifactoryWithoutInternal() {
        ...
        try (MockedStatic<ArtifactoryRegistry> artifactory = MockRegistries.artifactoryRegistry("artifactory.swg-devops.com", null, true);
            ...
            testMap.put(DockerImageName.parse("cloudant:1.0"), DockerImageName.parse("cloudant:1.0"));
            testMap.put(DockerImageName.parse("localhost/openliberty/testcontainers/kerberos:6.6"),
                        DockerImageName.parse("localhost/openliberty/testcontainers/kerberos:6.6"));
    ...
    public void testWithoutArtifactoryWithInternal() {
        ...
        try (MockedStatic<ArtifactoryRegistry> artifactory = MockRegistries.artifactoryRegistry("", null, false);
            ...
            Map<DockerImageName, DockerImageName> testMap = new HashMap<>();
            // Images that exist in an artifactory mirror only
            testMap.put(DockerImageName.parse("ghcr.io/oracle:1.0"), DockerImageName.parse("ghcr.io/oracle:1.0"));
            testMap.put(DockerImageName.parse("icr.io/open-liberty:2.0"), DockerImageName.parse("icr.io/open-liberty:2.0"));
            testMap.put(DockerImageName.parse("public.ecr.aws/postgres:1.14.2"), DockerImageName.parse("public.ecr.aws/postgres:1.14.2"));
            ...
            for (Map.Entry<DockerImageName, DockerImageName> test : testMap.entrySet()) {
                DockerImageName expected = test.getValue();
                DockerImageName actual = substitutor.apply(test.getKey());
                assertEquals("Only images supported by an internal mirror should have been substituted",
                             expected, actual);
            }
        }
    ...
    public void testWithoutArtifactoryWithoutInternal() {
        ...
        //Images that do not exist in either registry (synthetic or committed)
        testMap.put(DockerImageName.parse("sha256:5103a25d3efd8c0cbdbc80d358c5b1da91329c53e1fa99c43a8561a87eb61d3b"),
                    DockerImageName.parse("sha256:5103a25d3efd8c0cbdbc80d358c5b1da91329c53e1fa99c43a8561a87eb61d3b"));
        testMap.put(DockerImageName.parse("localhost/testcontainers/ryuk:latest"),
                    DockerImageName.parse("localhost/testcontainers/ryuk:latest"));
        ...
        for (Map.Entry<DockerImageName, DockerImageName> test : testMap.entrySet()) {
            DockerImageName expected = test.getValue();
            DockerImageName actual = substitutor.apply(test.getKey());
            assertEquals("Images should have been the same after substitution when no registry is available",
                         expected, actual);
        }
