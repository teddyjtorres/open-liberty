...
public abstract class Registry {
    ...
    protected static String findRegistryUser(final String registryUserKey) throws IllegalStateException {
        Log.info(c, "findRegistryUser", "Searching system property " + registryUserKey + " for a registry user.");
        String user = System.getProperty(registryUserKey);
        if (user == null || user.isEmpty() || user.startsWith("${") || user.equals("null")) {
            throw new IllegalStateException("No registry user was configured. System property '" + registryUserKey + "' was: " + user
                                            + ". Ensure registry properties are set on system.");
        }
        return user;
    }

    protected static String findRegistryPassword(final String registryPasswordKey) throws IllegalStateException {
        Log.info(c, "findRegistryPassword", "Searching system property " + registryPasswordKey + " for a registry password.");
        String registry = System.getProperty(registryPasswordKey);
        if (registry == null || registry.isEmpty() || registry.startsWith("${") || registry.equals("null")) {
            throw new IllegalStateException("No registry was configured. System property '" + registryPasswordKey + "' was unset "
                                            + ". Ensure registry properties are set on system.");
        }
        return registry;
    }
